/*
    (c) 2016. All rights reserved.
    Long strings
*/

export const DRAW_WARN = "Implementations of canvas component must overwrite 'draw' method.";
export const IMAGE_LOADED = "woo yey image loaded!";
export const IMAGE_ERROR = "No, nothing worked!";

export const CANVAS = 'canvas';
export const ROOT_ELEMENT = 'root';
export const MUSIC_KNOB = 'music_knob';
export const MUSIC_KNOB_CANVAS = 'music_knob_canvas';
export const MUSIC_KNOB_OFF_CANVAS = 'music_knob_off_canvas';
export const MUSIC_KNOB_IMAGE_URL = 'knob_image.png';
export const MUSIC_KNOB_SIZE = 400;
export const ROTATION = 'rotation';

export const ReactNum = React.propTypes.number;
